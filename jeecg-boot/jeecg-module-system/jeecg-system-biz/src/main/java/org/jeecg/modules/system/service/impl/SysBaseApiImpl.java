...
public class SysBaseApiImpl implements ISysBaseAPI {
	...
	public List<SysPermissionDataRuleModel> queryPermissionDataRule(String component, String requestPath,
		...
		if (oConvertUtils.isNotEmpty(component)) {
			...
			if (currentSyspermission == null || currentSyspermission.size() == 0) {
				...
				if (regUrl != null) {
					currentSyspermission = sysPermissionMapper.selectList(new LambdaQueryWrapper<SysPermission>()
							.eq(SysPermission::getMenuType, 2).eq(SysPermission::getUrl, regUrl).eq(SysPermission::getDelFlag, 0));
				}
			}
		}
		if (currentSyspermission != null && currentSyspermission.size() > 0) {
	...
	public void updateSysAnnounReadFlag(String busType, String busId) {
		...
		if (announcement != null) {
			...
			// updateWrapper.last("where annt_id ='"+announcement.getId()+"' and user_id
			// ='"+userId+"'");
			SysAnnouncementSend announcementSend = new SysAnnouncementSend();
	...
	public List<JSONObject> queryUserBySuperQuery(String superQuery, String matchType) {
		...
		if (ObjectUtils.isNotEmpty(list)) {
			...
			// update-begin-author:taoyan date:2023-5-19 for: QQYUN-5326【简流】获取组织人员 单/多 筛选条件
			// 没有部门筛选
			...
			for (SysUser user : list) {
				...
				// update-end-author:taoyan date:2023-5-19 for: QQYUN-5326【简流】获取组织人员 单/多 筛选条件
				// 没有部门筛选
	...
	public boolean hasOnlineAuth(OnlineAuthDTO onlineAuthDTO) {
		...
		if (permissionList == null || permissionList.size() == 0) {
			...
			if (count <= 0) {
				// update-begin---author:chenrui ---date:20240123
				// for：[QQYUN-7992]【online】工单申请下的online表单，未配置online表单开发菜单，操作报错无权限------------
				...
				// update-end---author:chenrui ---date:20240123
				// for：[QQYUN-7992]【online】工单申请下的online表单，未配置online表单开发菜单，操作报错无权限------------
			}
		} else {
			// 找到菜单了
	 ...
	 */
	@Override
	public void sendHtmlTemplateEmail(String email, String title, EmailTemplateEnum emailTemplateEnum,
			JSONObject params) {
		EmailSendMsgHandle emailHandle = new EmailSendMsgHandle();
		String htmlText = "";
		try {
	...
	public Map<String, String> copyLowAppDict(String originalAppId, String appId, String tenantId) {
		...
		log.info(" --- 批量复制应用下的字典到新租户下 —— 原应用ID:{}，新应用ID:{}，新租户ID：{}，字典个数：{} --- ", originalAppId, appId, tenantId,
				dictCodeMapping.size());
	...
	private List<String> getOnlinePossiblePaths(String path) {
		...
		log.info(" path = " + path);
		if (path.indexOf("?") >= 0 && (path.contains("/online/cgreport/") || path.contains("/online/cgformList/")
				|| path.contains("/online/graphreport/"))) {
			// 包含?说明有多个参数
