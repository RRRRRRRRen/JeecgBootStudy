...
public class SysDictServiceImpl extends ServiceImpl<SysDictMapper, SysDict> implements ISysDictService {
	...
	public boolean duplicateCheckData(DuplicateCheckVo duplicateCheckVo) {
		...
		// 4.执行SQL 查询是否存在值
		try {
			// update-begin---author:chenrui ---date:20240715
			// for：[TV360X-49]postgres日期、年月日时分秒唯一校验报错------------
			...
			// update-end---author:chenrui ---date:20240715
			// for：[TV360X-49]postgres日期、年月日时分秒唯一校验报错------------
	...
	public Map<String, List<DictModel>> queryDictItemsByCodeList(List<String> dictCodeList) {
		...
		for (DictModelMany dict : list) {
			...
			// update-begin-author:taoyan date:2023-4-28 for: QQYUN-5183【简流】多字段拼接-多选框、下拉框
			// 等需要翻译的字段
			// dict.setDictCode(null);
			// update-end-author:taoyan date:2023-4-28 for: QQYUN-5183【简流】多字段拼接-多选框、下拉框
			// 等需要翻译的字段
	...
	public String queryTableDictTextByKey(String table, String text, String code, String key) {
		...
		List<DictModel> dictModeList = sysDictMapper.queryTableDictByKeysAndFilterSql(table, text, code, null,
				Arrays.asList(key));
	...
	public List<DictModel> queryTableDictTextByKeys(String table, String text, String code, List<String> codeValues,
			String dataSource) {
		String str = table + "," + text + "," + code;
		// update-begin---author:chenrui ---date:20231221
		// for：[issues/#5643]解决分布式下表字典跨库无法查询问题------------
		// 是否自定义数据源
		...
		// update-end---author:chenrui ---date:20231221
		// for：[issues/#5643]解决分布式下表字典跨库无法查询问题------------
	 ...
	 */
	@Override
	public List<String> queryTableDictByKeys(String table, String text, String code, String codeValuesStr,
			boolean delNotExist) {
		...
		// 5.查询字典数据
		List<DictModel> dicts = sysDictMapper.queryTableDictByKeysAndFilterSql(
				SqlInjectionUtil.getSqlInjectTableName(table),
				SqlInjectionUtil.getSqlInjectField(text), SqlInjectionUtil.getSqlInjectField(code), filterSql,
				Arrays.asList(codeValues));

		List<String> texts = new ArrayList<>(dicts.size());
		// 6.查询出来的顺序可能是乱的，需要排个序
		for (String conditionalVal : codeValues) {
			List<DictModel> res = dicts.stream().filter(i -> conditionalVal.equals(i.getValue()))
	...
	@Override
	public List<DictModel> queryLittleTableDictItems(String tableSql, String text, String code, String condition,
	...
	private String getFilterSql(String tableSql, String text, String code, String condition, String keyword) {
		...
		// 下拉搜索组件 支持传入排序信息 查询排序
		// update-begin---author:chenrui ---date:20240327 for：[QQYUN-8514]Online表单中
		// 下拉搜索框 搜索时报sql错误，生成的SQL多了一个 “and" ------------
		...
		// update-end---author:chenrui ---date:20240327 for：[QQYUN-8514]Online表单中 下拉搜索框
		// 搜索时报sql错误，生成的SQL多了一个 “and" ------------
		// 增加排序逻辑
	...
	public List<SysDict> queryDeleteList(String tenantId) {
		// update-begin---author:wangshuai---date:2024-02-27---for:【QQYUN-8340】回收站查找软删除记录时，没有判断是否启用多租户，造成可以查找并回收其他租户的数据
		// #5907---
		if (MybatisPlusSaasConfig.OPEN_SYSTEM_TENANT_CONTROL) {
		...
		// update-end---author:wangshuai---date:2024-02-27---for:【QQYUN-8340】回收站查找软删除记录时，没有判断是否启用多租户，造成可以查找并回收其他租户的数据
		// #5907---
