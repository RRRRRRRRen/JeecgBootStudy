...
public class SysUserController {
    ...
    /**
     * TODO 未知路径
     * * 从配置文件中读取配置项的值，并注入到对应的字段（属性）中。
     * 
     * * 让 Spring 从配置文件（如 application.properties 或 application.yml）中，查找键为
     * jeecg.path.upload 的值。
     * *
     */
    ...
    public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
        ...
        // * 获取参数 selecteddeparts
        String selectedDeparts = jsonObject.getString("selecteddeparts");
        try {
            // * 将 JSONObject 转化为 SysUser
    ...
    public Result<SysUser> edit(@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            if (sysUser == null) {
                 ...
                 * * 转化为java对象
                 * 
                 * * - 更推荐 SysUser user = jsonObject.toJavaObject(SysUser.class);
                 * * - 或者 @RequestBody SysUser user 直接映射
                 */
                SysUser user = JSON.parseObject(jsonObject.toJSONString(), SysUser.class);
                // * 设置更新时间
                ...
                // * 使用 selecteddeparts || departIds 作为部门数据
    ...
    public Result<?> queryByOrgCodeForAddressList(
        ...
        for (SysUserSysDepartModel item : list) {
            ...
            if (getModel != null) {
                // * 之前已存在过该用户，直接合并departName数据，但是貌似缺少departId的合并
    ...
    public Result<SysUserDepart> deleteUserInDepart(
    ...
    public Result<SysUser> passwordChange(
         ...
         */
        if (oConvertUtils.isEmpty(username)
                || oConvertUtils.isEmpty(password)
                || oConvertUtils.isEmpty(smscode)
    ...
    public Result<?> getUserSectionInfoByToken(
    ...
    public Result<SysUser> saveClientId(
            HttpServletRequest request,
        ...
        try {
            ...
            if (sysUser == null) {
                ...
                /**
                 * * 这种写法不太好
                 * * - 使用 .set(SysUser::getClientId, dto.getClientId())); 去更新比较好
                 */
                sysUserService.updateById(sysUser);
            }
        } catch (Exception e) {
    ...
    public Result<List<SysUser>> appQueryUser(
            @RequestParam(name = "keyword", required = false) String keyword,
            @RequestParam(name = "username", required = false) String username,
            @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo,
            @RequestParam(name = "pageSize", defaultValue = "10") Integer pageSize,
