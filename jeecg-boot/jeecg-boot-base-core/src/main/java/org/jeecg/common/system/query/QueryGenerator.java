...
public class QueryGenerator {
	...
	// update-begin---author:chenrui ---date:20240527
	// for：[TV360X-378]增加自定义字段查询规则功能------------
	/**
	 * 获取查询条件构造器QueryWrapper实例 通用查询条件已被封装完成
	 * 
	 * @param searchObj     查询实体
	...
	// update-end---author:chenrui ---date:20240527
	// for：[TV360X-378]增加自定义字段查询规则功能------------
	 ...
	 * QueryWrapper<JeecgDemo>(jeecgDemo);
	 * <br>
	 * 正确示例:QueryWrapper<JeecgDemo> queryWrapper = new QueryWrapper<JeecgDemo>();
	 * <br>
	...
	private static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMap,
		 ...
		 * 当一个人只有一个部门 就直接配置等于条件: orgCode 等于 #{sys_org_code} 或者配置自定义SQL: orgCode =
		 * '#{sys_org_code}'
		 */
	...
	private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap,
		...
		if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
			...
			// 修改为 _ 前端传递排序条件{....,column: 'column1,column2',order: 'desc'} 翻译成sql "column1
			// desc,column2 desc"
	...
	// update-begin-author:taoyan date:2022-5-23 for: issues/3676
	// 获取系统用户列表时，使用SQL注入生效
	/**
	 * 多字段排序 判断所传字段是否存在
	 * 
	...
	private static void doSuperQuery(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap,
		...
		if (parameterMap != null && parameterMap.containsKey(SUPER_QUERY_PARAMS)) {
			...
			String superQueryMatchType = parameterMap.get(SUPER_QUERY_MATCH_TYPE) != null
					? parameterMap.get(SUPER_QUERY_MATCH_TYPE)[0]
			...
			try {
				...
				// update-begin-author:sunjianlei date:20220119 for: 【JTC-573】 过滤空条件查询，防止 sql
				// 拼接多余的 and
				...
				// update-end-author:sunjianlei date:20220119 for: 【JTC-573】 过滤空条件查询，防止 sql
				// 拼接多余的 and
	...
	public static QueryRuleEnum convert2Rule(Object value) {
		...
		// update-end-author:taoyan date:20210629 for: 查询条件输入空格导致return
		// null后续判断导致抛出null异常
	...
	public static void addEasyQuery(QueryWrapper<?> queryWrapper, String name, QueryRuleEnum rule, Object value) {
		...
		switch (rule) {
			...
			case IN:
				...
				// update-begin-author:taoyan date:20200909 for:【bug】in 类型多值查询 不适配postgresql
				// #1671
			...
			case NOT_RIGHT_LIKE:
				...
				break;
			// update-begin---author:chenrui ---date:20240527
			// for：[TV360X-378]下拉多框根据条件查询不出来:增加自定义字段查询规则功能------------
			case LIKE_WITH_OR:
	...
	private static boolean judgedIsUselessField(String name) {
		...
		//// update-begin--author:sunjianlei date:20240808 for：【TV360X-2009】取消过滤
		//// sort、order 字段，防止前端排序报错 ------
		//// https://github.com/jeecgboot/JeecgBoot/issues/6937
		// || "sort".equals(name) || "order".equals(name)
		//// update-end----author:sunjianlei date:20240808 for：【TV360X-2009】取消过滤
		//// sort、order 字段，防止前端排序报错 ------
		;
	}
	...
	public static Map<String, SysPermissionDataRuleModel> getRuleMap() {
		...
		// update-begin-author:taoyan date:2023-6-1 for:QQYUN-5441 【简流】获取多个用户/部门/角色
		// 设置部门查询 报错
		try {
		...
		// update-end-author:taoyan date:2023-6-1 for:QQYUN-5441 【简流】获取多个用户/部门/角色 设置部门查询
		// 报错
	...
	private static void addRuleToQueryWrapper(SysPermissionDataRuleModel dataRule, String name, Class propertyType,
		...
		if (rule.equals(QueryRuleEnum.IN) && !propertyType.equals(String.class)) {
			...
			if (propertyType.equals(String.class)) {
				...
				// update-begin---author:chenrui ---date:20241125 for：[issues/7481]多租户模式下
				// 数据权限使用变量：#{tenant_id} 报错------------
				addEasyQuery(queryWrapper, name, rule,
						NumberUtils.parseNumber(converRuleValue(dataRule.getRuleValue()), propertyType));
				// update-end---author:chenrui ---date:20241125 for：[issues/7481]多租户模式下
				// 数据权限使用变量：#{tenant_id} 报错------------
			}
		}
	}
	...
	public static Set<String> getSqlRuleParams(String sql) {
		...
		// update-begin---author:chenrui ---date:20250108
		// for：[QQYUN-10785]数据权限，查看自己拥有部门的权限中存在问题 #7288------------
		String regex = "#\\{\\[*\\w+]*}";
		// update-end---author:chenrui ---date:20250108
		// for：[QQYUN-10785]数据权限，查看自己拥有部门的权限中存在问题 #7288------------
	...
	public static String installAuthJdbc(Class<?> clazz) {
		...
		for (int i = 0; i < origDescriptors.length; i++) {
			...
			if (ruleMap.containsKey(name)) {
				...
				Object value;
				// update-begin---author:chenrui ---date:20240527
				// for：[TV360X-539]数据权限，配置日期等于条件时后端报转换错误------------
				...
				String filedSql = SqlConcatUtil.getSingleSqlByRule(rule, oConvertUtils.camelToUnderline(column), value,
						isString);
